...
lambda_birth = lambda startyear, endyear: str(random_number(startyear, endyear)) + '-' + str(
    random_number(1, 12)) + '-' + str(random_number(1, 32))
lambda_birth_ohne_ = lambda startyear, endyear: str(random_number(startyear, endyear)) + str(
    random_number(1, 12)) + str(random_number(1, 32))
...
class UserService(object):
    def __init__(self):
        ...
        engine = create_engine(
            "mysql+pymysql://root:root@localhost:3306/bsdb?charset=utf8mb4")
        input_password = "12qw"
    ...
    def create_user(self) -> []:
        ...
        password = CryptContext(schemes=["bcrypt"], deprecated="auto").hash(
            input_password)  # 백엔드에서 실행할 경우 pip install bcrypt 필요
